...
PHPUnit against a real WordPress test suite with GiveWP installed; MySQL runs in
Docker (`tests/docker-compose.yml`). Test files:

- [tests/Unit/HandleReturnUrlTest.php](tests/Unit/HandleReturnUrlTest.php)
- [tests/Unit/WebhookNotificationsListenerTest.php](tests/Unit/WebhookNotificationsListenerTest.php)
- [tests/Unit/WayforpayGatewayTest.php](tests/Unit/WayforpayGatewayTest.php)

```bash
composer install        # also copies tests/wp-tests-config.dist.php → wp-tests-config.php
composer test           # brings docker up, runs phpunit, tears docker down
composer docker:up      # start MySQL only
composer docker:down    # stop MySQL
```

The `webhookNotificationsListener` throws `\WPDieException` (instead of `exit`) when
that class exists, so the ack path can be asserted in tests.

## Building a release

`composer zip` installs prod-only dependencies, produces `wayforpay-givewp.zip`
(excluding dev/test/tooling files), then restores dev dependencies.
